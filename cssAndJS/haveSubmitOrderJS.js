...
function checkOrder(){
    var url = myUrl+"sendmanagerServiceController.do?datagrid";
    var JSONP =document.createElement("script");
    JSONP.type="text/javascript";
    JSONP.src=url+ "&sessionid=" + localStorage.sessionid + "&sendid=" + document.getElementById("ordercodeInput").value + "&fstatus=Y" + "&billingdate_begin=" + document.getElementById("beforeDate").value + "&billingdate_end=" + document.getElementById("nowDate").value + "&callback=jsonpcallback";
...
function creatNode(){
    ...
    if (!orderSummary.asterntime && !orderSummary.begintime && !orderSummary.endtime){
        span.innerHTML = "未到";
    }
    if (orderSummary.asterntime){
        span.innerHTML = "正在排队";
    }
    if (orderSummary.begintime){
        span.innerHTML = "开始装车";
    }
    if (orderSummary.endtime){
        span.innerHTML = "装车完毕";
    }
    btn1.name = i;
    btn2.name = i;
...
function jsonpcallback2(result) {
    if(result.msgDesc == "需要重新登录！"){
        ...
        if (result.msgDesc == "派车主表删除成功") {
            window.location.href = "wantDelectSumbitOrder.html";
        } else {
